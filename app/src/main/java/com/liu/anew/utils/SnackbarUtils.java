...
public final class SnackbarUtils {
    ...
    public void showTop() {
        ...
        if (bottomMargin != 0) {
            int height = ScreenUtil1.getScreenHeight(MyApplication.mContext);
            int statusHeight = ScreenUtil1.getStatusHeight(MyApplication.mContext);
            int actionBarHeight = ScreenUtil1.getActionBarHeight(MyApplication.mContext);
            ViewGroup.MarginLayoutParams params =
                    (ViewGroup.MarginLayoutParams) snackbarView.getLayoutParams();
            params.setMargins(0,0,0,height-statusHeight-actionBarHeight);;
        }
        if (actionText.length() > 0 && actionListener != null) {
            if (actionTextColor != COLOR_DEFAULT) {
                snackbar.setActionTextColor(actionTextColor);
